...
<?php
	$informations = $db->table('support')->where('to_admin','=',0)->where('id','=',m_a_g('id'))->get();
...
<div class="collapse navbar-collapse" id="navbar-component">
	<ul class="navbar-nav">
		<li class="nav-item">
			<a href="<?php echo MODERATOR_URL;?>/index.php?page=support&status=0" class="navbar-nav-link">Destek Talepleri</a>
		</li>
		<li class="nav-item">
...
if($_POST)
{
	...
	if(m_a_p('type')=='to_admin')
	{
		...
		m_redirect(MODERATOR_URL.'/index.php?page=support&status=0');
	}
...
<form action="" method="post" enctype="multipart/form-data">
		<input type="hidden" name="type" value="to_admin">
		<div>
		<button type="submit" class="btn btn-primary btn-block mt-2 mb-2">Yetkiliye Yönlendir</button>
		</div>
</form>
	  ...
	  <?php
		  ...
		  <li class="list-group-item d-flex justify-content-between align-items-start">
			<div class="ms-2 me-auto">
			  ...
			  <a href="<?php echo MODERATOR_URL; ?>/index.php?page=review_detail&id=<?php echo $review_info['id']; ?>" target="_blank"><?php echo $review_info['title']; ?></a>
			</div>
		  </li>
	  <?php
			}
