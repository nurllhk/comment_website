...
{
	var client_height = $('.chat_message_list')[0].clientHeight;
	var scroll_top = $('.chat_message_list').scrollTop();
	var current_scroll = scroll_top + client_height;
	var scroll_height = $('.chat_message_list').prop("scrollHeight");
...
function chat_scroll(type=false)
{
	...
	if(type || chat_last_scroll)
	{
		...
		$('.chat_message_list').scrollTop(scroll_height);
		
	}
}

function chat_message_list(scroll=false)
{
		$.ajax({
            ...
            url: 'ajax.php',
			data: 'type=chat_message_list',
			dataType: 'json',
			cache:false,
            success: function(data) {

						chat_scroll_before();
						$('.chat_message_list').html(data.chat_message_list);
						chat_scroll(scroll);
						chat_info_interval = window.setTimeout(chat_message_list, 5000);
            }
...
$(document).on("click", '.bulk_delete_btn', function (e) {
	e.preventDefault();
		swal({
		title: "Emin misiniz ?",
		text: "Bu bu içerikleri silmek istediğinizden eminmisiniz ?",
		type: "warning",
		showCancelButton: true,
		confirmButtonClass: 'btn btn-success',
	   cancelButtonClass: 'btn btn-danger',
		...
		cancelButtonText: "Hayır"
			  ...
			  else
			  {
				  if(e.value)
				  {
					var form_data = $('.bulk_delete').serialize();
					$.ajax({
					type: 'POST',
					url: 'ajax.php',
					data: 'type=bulk_delete_product&'+form_data,
					success: function(data) 
					{
...
$(document).on("click", '.bulk_check_btn', function (event) {
	
		if ($('input.bulk_check').is(':checked')) {
...
$(document).on("change", '.balance_filter', function (event) {
		...
		table.tables(['#balance_table']).columns(1).search($(this).val()).draw();
...
if ($('.balances').length)
{
			$.ajax({
				...
				success: function(data) {
					...
					$('.today_total').html('<h5 class="font-weight-semibold mb-0">'+data.today_total+' <i class="fa fa-lira-sign"></i></h5>');
					$('.today_best_user').html('<h5 class="font-weight-semibold mb-0">'+data.today_best_user+'</h5>');
					$('.today_best_user_total').html('<span class="text-uppercase font-size-sm text-muted">'+data.today_best_user_total+' <i class="fa fa-lira-sign"></i></span>');
					$('.today_best_type').html('<h5 class="font-weight-semibold mb-0">'+data.today_best_type+'</h5>');
					$('.today_best_type_total').html('<span class="text-uppercase font-size-sm text-muted">'+data.today_best_type_total+' <i class="fa fa-lira-sign"></i></span>');
					
					$('.today_review_first_pay').html('<h5 class="font-weight-semibold mb-0">'+data.today_review_first_pay+' <i class="fa fa-lira-sign"></i></h5>');
					$('.today_review_pay').html('<h5 class="font-weight-semibold mb-0">'+data.today_review_pay+' <i class="fa fa-lira-sign"></i></h5>');
					$('.today_time_pay').html('<h5 class="font-weight-semibold mb-0">'+data.today_time_pay+' <i class="fa fa-lira-sign"></i></h5>');
					$('.today_referer_withdraw_amount').html('<h5 class="font-weight-semibold mb-0">'+data.today_referer_withdraw_amount+' <i class="fa fa-lira-sign"></i></h5>');
				}
			})
}
...
table = $('.datatable-button-html5-basic').DataTable({
					charset: "UTF-8",
          buttons: {            
                buttons: [
                    {
