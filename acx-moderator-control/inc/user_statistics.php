...
<?php
	$register_dates = $db->query('select COUNT(*) as total,register_date from users group by DATE(register_date) order by register_date asc')->fetchAll(PDO::FETCH_ASSOC);
	$dates = '';
	foreach($register_dates  as $date)
	{
		$dates.='\''.date('d/m/Y',strtotime($date['register_date'])).'\',';
		$totals.=''.$date['total'].',';
	}
	$dates = trim($dates,',');
	$totals = trim($totals,',');
...
<div class="collapse navbar-collapse" id="navbar-component">
	<ul class="navbar-nav">
		<li class="nav-item">
			<a href="<?php echo MADMIN_URL;?>" class="navbar-nav-link">Anasayfa</a>
		</li>
		<li class="nav-item">
			<a href="#" class="navbar-nav-link"><i class="fa fa-share"></i></a>
		</li>
		<li class="nav-item">
			<a href="#" class="navbar-nav-link">Kullanıcı Kayıt İstatistikleri</a>
		</li>
	</ul>
...
<h5 class="card-title">Kullanıcı Kayıt İstatistikleri</h5>
</div>
