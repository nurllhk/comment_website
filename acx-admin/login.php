					...
					<div class="card mb-0">
						<div class="card-body">
							<div class="text-center mb-3">
								<br>
								<h5 class="mb-0">Yetkili Giriş</h5>
							</div>
							<?php
							if($_POST)
							{
								
								if($db->table('admins')->where('email','=',m_a_p('email'))->where('password','=',m_password(m_a_p('password')))->count()==0)
								{
									echo m_alert('Hata','Geçersiz kullanıcı bilgileri.');
								}
								else
								{	
									$login_id = $db->table('admins')->where('email','=',m_a_p('email'))->where('password','=',m_password(m_a_p('password')))->get_var('id');
									$data = [
									'last_login' => $db->now(),
									'last_ip' => m_ip()
									];
									$query = $db->table('admins')->where('id','=',$login_id)->update($data);
