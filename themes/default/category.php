<?php
	...
	if($category['c_id']!=0)
	{ 
		$top_category = $db->table('categories')->where('id','=',$category['c_id'])->get_var('title').' - '; 
	}
...
<div class="container">
	...
	<div class="page_title"><h1><?php echo $top_category; ?><?php echo $category['title']; ?></h1></div>
	</div>
	...
	<?php
			...
			$review_list->query_options 			= "where p.c_id LIKE '%[".$category['id']."]%' and r.status='1'";
	...
	if($result['count']==0)
	{
		echo m_alert('Bilgi','Bu kategori için şuanda bir inceleme bulunmuyor.');
	}
	?>
	</div>
	<?php
	if(m_u_g('order')=='')
	{
		echo m_pagination($result['total_page'],$result['current_page'],m_permalink('category',$category['sef'],$category['id']));
	}
	else
	{
		echo m_pagination($result['total_page'],$result['current_page'],m_permalink('category',$category['sef'],$category['id']).'/sira/puan');
	}
	?>
	<?php
